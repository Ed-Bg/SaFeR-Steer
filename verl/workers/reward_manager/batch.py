...
class BatchRewardManager(AbstractRewardManager):
    ...
    def verify(self, data):
        ...
        # 🔧 Fix: Get turn_scores directly from non_tensor_batch (from agent_loop extra_fields)
        # turn_scores_batch = data.non_tensor_batch.get("turn_scores", [None for _ in range(len(data))])

        for i in range(len(data)):
            extras[i]["rollout_reward_scores"] = rollout_reward_scores[i]
            tool_extra = tool_extra_fields[i]
            if isinstance(tool_extra, dict):
                extras[i].update(tool_extra)
            # 🔧 Fix: Add turn_scores to extra_info
            # if turn_scores_batch[i] is not None:
                # extras[i]["turn_scores"] = turn_scores_batch[i]
