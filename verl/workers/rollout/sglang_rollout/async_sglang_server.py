...
def _remove_video_pad_tokens(prompt_ids: list[int], video_token_id: int) -> list[int]:
    """Remove video_pad tokens from prompt_ids when no video data is provided.
    
    This prevents sglang from trying to find video data for video_pad tokens.
    
    Args:
        prompt_ids: List of token ids
        video_token_id: The token id for <|video_pad|>
        
    Returns:
        List of token ids with video_pad tokens removed
    """
    if video_token_id is None:
        return prompt_ids
    prompt_ids_arr = np.array(prompt_ids)
    mask = prompt_ids_arr != video_token_id
...
class SGLangHttpServer:
    ...
    async def generate(
        ...
        # Remove video_pad tokens to prevent sglang from trying to find video data for these tokens.
        # This is needed because:
        # 1. sglang currently doesn't support video input (video_data is not passed to the request)
        # 2. The model may generate video_pad tokens during generation, which would cause errors
        #    when the accumulated prompt_ids are sent to sglang in subsequent turns.
        video_token_id = getattr(self.model_config.hf_config, "video_token_id", None)
        if video_token_id is not None:
            prompt_ids = _remove_video_pad_tokens(prompt_ids, video_token_id)
